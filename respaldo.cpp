...
// Estructura de los paquetes y tokens  TAMAÑO 16 BYTES
struct paquete {
...
//Creación de token de validación 1
...
//Creación de ACK de red validada
...
//Creación de token de descubrimiento 2
...
//Creación de token de publicacion 3  7 bytes
...
void respuesta(struct paquete message){
    ...
    switch (message.tipo) {
        ...
        case '3':
            printf("Token de publicación");
            if (!propietario( message.dest)) {
                nodos = message.contenido[0]- 96;
                printf("Nodos %i", nodos);
                reenvio_paquete(message);
            }else{
                printf("Token de publicación terminado");
                printf("Nodos %i", nodos);
            }
